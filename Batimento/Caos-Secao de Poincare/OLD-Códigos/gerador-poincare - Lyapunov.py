...
def Simular(nome,r0, yevent):
    ...
    salvar = {'r0': r0, 'rs': rs, 'vs': vsf, 't': ts, 'tpic': sol.t_events[0], 'rpic': np.transpose(sol.y_events[0])[:2,:], 'vpic': np.transpose(sol.y_events[0])[2:,:]}
...
if __name__ == '__main__':
    ...
    DL = 1e-8 #variação na posição para calculo do expoente de Lyapunov
    ...
    nomesL = [f'{diretorio}\\dadoL-{i}' for i in range(len(z0s))]
