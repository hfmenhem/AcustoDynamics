...
U2 = sim.UGorkov(Pt, GPt)
U2 = U2.reshape(len(ys),(len(zs)))
...
U2F = np.where(np.reshape(Filtro, (200,400)),U2, nan)
...
fig42, ax42 = plt.subplots(dpi=300, figsize=(10,10))
ax42.set_aspect(1)
#pcm = ax.pcolor(zs, zs, FscnF,norm=colors.LogNorm(vmax=FscnF.max()))
#pcm4 = ax4.pcolormesh(ys, zs, (10**9)*np.transpose(UTrF), norm=colors.SymLogNorm(linthresh=10**7))
pcm42 = ax42.contourf(ys, zs, (10**9)*np.transpose(U2F),levels=40)

fig42.colorbar(pcm42, ax=ax42, label='Ut [aJ]')
ax42.set_title('Potencial de Gor\'kov total - novo')
ax42.set_xlabel('y [mm]')
ax42.set_ylabel('z [mm]')
fig42.show()
